...
class RandomRolloutEvaluator(Evaluator):
  ...
  def evaluate(self, state, root_eval):
    """Returns evaluation on given state."""
    result = None
...
class SearchNode(object):
  ...
  def __init__(self, action, player, prior, state = []):
    self.action = action
    self.player = player
    self.prior = prior
...
class MCTSBot(pyspiel.Bot):
  ...
  def step_with_policy(self, state):
    ...
    t1 = time.time()
    root, root_eval = self.mcts_search(state)
    ...
    return policy, mcts_action, root_eval

  def step(self, state):
    return self.step_with_policy(state)[1], self.step_with_policy(state)[2]

  def _apply_tree_policy(self, root, root_eval, state):
    ...
    while (not working_state.is_terminal() and
      ...
      if not current_node.children:
        ...
        current_node_eval.children = [
            SearchNode(action, player, prior, state = current_node_eval.state + [action]) for action, prior in legal_actions
        ]
      ...
      if working_state.is_chance_node():
        ...
        chosen_action = chosen_child.action
        for c in current_node_eval.children:
          if c.action == chosen_action:
            current_node_eval = c
  ...
  def mcts_search(self, state):
    ...
    for _ in range(self.max_simulations):
      ...
      if working_state.is_terminal():
        ...
        returns = self.evaluator.evaluate(working_state, current_node_eval)
        solved = False
